    ...
    def get_data_by_url(self,url):
        ...
        for link in auth_link:
            ...
            if len(picture)==0:
                ...
                pic.append(picture)
        for i in range(0,7):
            image_floder="./images/"+auth_name[i]
            for image in pic[i]:
                image_content=requests.get(image,headers=self.headers,verify=False)
                file_name=image_floder+"/"+auth_name[i]+".jpg"
                image_paths.append(file_name)
            with open(file_name,'wb') as f:
                f.write(image_content.content)
            # hero_index+=1
        print(auth_name[i]+"下载完毕")
        # index+=1
